...
def send_message(chat_id_list, ticket_id, message, silent=False):
    for chat_id in chat_id_list:
        ...
        if response.status_code in (200, 201):
            logger.info(f'send_message() is done for ticket_id={ticket_id}, chat_id={chat_id}, silent={silent}, '
                        f'status_code={response.status_code}')
        else:
            logger.warning(f'send_message() does not send for ticket_id={ticket_id}, chat_id={chat_id}, '
                           f'silent={silent}, status_code={response.status_code}')
...
def notify_new_ticket(ticket_id):
    ...
    send_message(list(chat_id_set), ticket_id, message)

    # Send with silent mode from 18 till 9 and all weekdays
    if Config.NOTIFY_START_TIME <= datetime.now().hour < Config.NOTIFY_STOP_TIME or datetime.now().weekday() in (5, 6):
        send_message(list(chat_id_silent_set), ticket_id, message, False)
    else:
        send_message(list(chat_id_silent_set), ticket_id, message, True)
...
def notify_new_task(ticket_id):
    ...
    send_message(list(task['chat_id_set']), ticket_id, message)
